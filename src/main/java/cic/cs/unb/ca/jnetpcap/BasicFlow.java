...
//import org.apache.commons.math3.stat.descriptive.SummaryStatistics;													// SRG
import org.apache.commons.math3.stat.descriptive.DescriptiveStatistics;													// SRG
import org.jnetpcap.packet.format.FormatUtils;

public class BasicFlow {

	private final static String separator = ",";
	private     DescriptiveStatistics 		fwdPktStats = null;															// SRG
	private		DescriptiveStatistics 		bwdPktStats = null;															// SRG
    ...
    private     DescriptiveStatistics flowIAT = null;																	// SRG
    private     DescriptiveStatistics forwardIAT = null;																// SRG
    private     DescriptiveStatistics backwardIAT = null;																// SRG
	private     DescriptiveStatistics flowLengthStats = null;															// SRG
    private     DescriptiveStatistics flowActive = null;																// SRG
    private     DescriptiveStatistics flowIdle = null;																	// SRG
	...
	public void initParameters(){
		...
		this.flowIAT = new DescriptiveStatistics();																		// SRG
		this.forwardIAT = new DescriptiveStatistics();																	// SRG
		this.flowActive = new DescriptiveStatistics();																	// SRG
		this.flowIdle = new DescriptiveStatistics();																	// SRG
		this.backwardIAT = new DescriptiveStatistics();																	// SRG
		this.flowLengthStats = new DescriptiveStatistics();																// SRG
		this.fwdPktStats = new DescriptiveStatistics();																	// SRG
		this.bwdPktStats =  new DescriptiveStatistics();																// SRG
	...
	public DescriptiveStatistics getFlowIAT() {	return flowIAT;	}														//SRG
	...
	public double getRelativeStd(double std, double mean) {																//SRG Relative Std
		if (mean == 0){
			return 0;
		}else {
			return std / mean;
	...
	public double getIQR(double q1, double q3) { return q3-q1;}															//SRG

	public double getFwdAvgBytesPerPacket(double packets, double bytes) {												//SRG
		if (packets == 0){
			return 0;
		} else {
	...
	public double getBwdAvgBytesPerPacket(double packets, double bytes) {												//SRG
		if (packets == 0){
			return 0;
		} else {
			return bytes / packets;
	...
	public double getAvgBytesPerPacket(double packets, double bytes) {												//SRG
		if (packets == 0){
			return 0;
		} else {
			return bytes / packets;
    ...
    public String dumpFlowBasedFeaturesEx() {
		...
		dump.append((long)packetCount()).append(separator);																//SRG
		dump.append((forwardBytes+backwardBytes)).append(separator);													//SRG
    	...
    	if(fwdPktStats.getN() > 0L) {
			...
			dump.append(getRelativeStd(fwdPktStats.getStandardDeviation(),fwdPktStats.getMean())).append(separator);	//SRG
			dump.append(fwdPktStats.getPercentile(50)).append(separator);											//SRG
			dump.append(getIQR(fwdPktStats.getPercentile(25),fwdPktStats.getPercentile(75))).append(separator);	//SRG
    	}else {
    		dump.append(0).append(separator);
    		dump.append(0).append(separator);
    		dump.append(0).append(separator);
    		dump.append(0).append(separator);
			dump.append(0).append(separator);																			//SRG
			dump.append(0).append(separator);																			//SRG
			dump.append(0).append(separator);																			//SRG
    	}
    	...
    	if(bwdPktStats.getN() > 0L) {
			...
			dump.append(getRelativeStd(bwdPktStats.getStandardDeviation(),bwdPktStats.getMean())).append(separator);	//SRG
			dump.append(bwdPktStats.getPercentile(50)).append(separator);											//SRG
			dump.append(getIQR(bwdPktStats.getPercentile(25),bwdPktStats.getPercentile(75))).append(separator);	//SRG
		}else{
    		dump.append(0).append(separator);
    		dump.append(0).append(separator);
    		dump.append(0).append(separator);
    	...
    	dump.append(getFlowBytesPerSec()).append(separator);						//21								//SRG
    	dump.append(getFlowPacketsPerSec()).append(separator);						//22								//SRG
		dump.append(getAvgBytesPerPacket(fwdPktStats.getN()+bwdPktStats.getN(),forwardBytes+backwardBytes)).append(separator);//SRG
		dump.append(getFwdAvgBytesPerPacket(fwdPktStats.getN(),forwardBytes)).append(separator);						//SRG
		dump.append(getBwdAvgBytesPerPacket(bwdPktStats.getN(),backwardBytes)).append(separator);						//SRG
		...
		dump.append(getRelativeStd(flowIAT.getStandardDeviation(),flowIAT.getMean())).append(separator);				//SRG
		dump.append(flowIAT.getPercentile(50)).append(separator);													//SRG
		dump.append(getIQR(flowIAT.getPercentile(25),flowIAT.getPercentile(75))).append(separator);				//SRG
    	...
    	if(this.forward.size()>1){
			...
			dump.append(getRelativeStd(forwardIAT.getStandardDeviation(),forwardIAT.getMean())).append(separator);		//SRG
			dump.append(forwardIAT.getPercentile(50)).append(separator);												//SRG
			dump.append(getIQR(forwardIAT.getPercentile(25),forwardIAT.getPercentile(75))).append(separator);		//SRG
    	...
    	if(this.backward.size()>1){
			...
			dump.append(getRelativeStd(backwardIAT.getStandardDeviation(),backwardIAT.getMean())).append(separator);	//SRG
			dump.append(backwardIAT.getPercentile(50)).append(separator);											//SRG
			dump.append(getIQR(backwardIAT.getPercentile(25),backwardIAT.getPercentile(75))).append(separator);	//SRG
    	}else{
    		dump.append(0).append(separator);
    		dump.append(0).append(separator);
    		dump.append(0).append(separator);
		...
		if(this.forward.size() > 0 || this.backward.size() > 0){
			...
			dump.append(getRelativeStd(flowLengthStats.getStandardDeviation(),flowLengthStats.getMean())).append(separator);//SRG
			dump.append(flowLengthStats.getPercentile(50)).append(separator);										//SRG
			dump.append(getIQR(flowLengthStats.getPercentile(25),flowLengthStats.getPercentile(75))).append(separator);//SRG
		...
		//dump.append(fHeaderBytes).append(separator);								//62 duplicate with 41
    	...
    	if(this.flowActive.getN()>0){
			...
			dump.append(getRelativeStd(flowActive.getStandardDeviation(),flowActive.getMean())).append(separator);		//SRG
			dump.append(flowActive.getPercentile(50)).append(separator);												//SRG
			dump.append(getIQR(flowActive.getPercentile(25),flowActive.getPercentile(75))).append(separator);		//SRG
    	...
    	if(this.flowIdle.getN()>0){
			...
			dump.append(getRelativeStd(flowIdle.getStandardDeviation(),flowIdle.getMean())).append(separator);			//SRG
			dump.append(flowIdle.getPercentile(50)).append(separator);												//SRG
			dump.append(getIQR(flowIdle.getPercentile(25),flowIdle.getPercentile(75))).append(separator);			//SRG
